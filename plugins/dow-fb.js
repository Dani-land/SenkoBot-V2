...
let handler = async (m, { conn, args, command, usedPrefix, text }) => {
  ...
  if (isFB) {
    ...
    // Validación kawaii
    if (!args[0])
      return conn.reply(m.chat, `🌸✨ *Senpai, por favor envíame un enlace de Facebook uwu*`, m, rcanal)

    if (!/facebook\.com|fb\.watch|web\.facebook\.com/.test(args[0]))
      return conn.reply(m.chat, `❌ *Ese enlace no es válido para Facebook, nyan~*`, m, rcanal)

    // Mensaje de descarga kawaii
